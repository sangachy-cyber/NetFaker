...
def train_model(train_file, val_file=None, epochs=10, batch_size=32, learning_rate=1e-4, output_dir="./results"):
    """训练模型"""
    # 创建输出目录
...
def visualize_training_losses(losses_file, output_dir="./results"):
    """可视化训练损失曲线"""
    # 加载训练损失
...
def sample_and_postprocess(model, scheduler_config_path, assets_dir, num_samples=10, output_dir="./results", device="cpu"):
    """采样并后处理生成的数据"""
    ...
    # 生成一些条件向量用于采样
    # 这里我们使用训练数据中的条件向量作为示例
    cond_vectors = []
    for i in range(num_samples):
        # 创建随机条件向量（这里只是示例，实际应用中应该使用真实的条件）
        cond = torch.randn(23)
        cond[11] = 0.0  # 网络状态ID设为0
...
def visualize_generated_samples(generated_samples_file, real_data_file, output_dir="./results"):
    """可视化生成的样本与真实数据的对比"""
    # 加载生成的样本
...
def main():
    ...
    generated_samples = sample_and_postprocess(
